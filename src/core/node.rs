...
impl Node {
    ...
    pub fn start(&mut self) -> Result<(), &'static str> {
        ...
        if let Some(ipv4) = self.cfg.ipv4() {
            let mut dht4 = Box::new(DHT::new(ipv4));
            if self.persistent {
                dht4.enable_persistence(&format!("{}/dht4.cache", self.storage_root));
                self.dht4 = Some(dht4);
            }
        }
        if let Some(ipv6) = self.cfg.ipv6() {
            let mut dht6 = Box::new(DHT::new(ipv6));
            if self.persistent {
    ...
    */
    ...
    pub fn find_node(&self, _: &Id, _: &LookupOption) -> Result<NodeInfo, &'static str> {
        unimplemented!()
    }
    ...
    pub fn find_peer(&self, _: &Id, _: i32, _:& LookupOption) -> Result<Vec<PeerInfo>, &'static str> {
        unimplemented!()
    }

    pub fn announce_peer(&self, _: &PeerInfo) -> Result<(), &'static str> {
        unimplemented!()
    }
}
