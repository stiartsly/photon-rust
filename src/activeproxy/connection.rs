...
pub(crate) struct ProxyConnection {
    ...
    session_keypair:Option<Rc<signature::KeyPair>>,
...
impl ProxyConnection {
    ...
    pub(crate) fn set_field<T: 'static>(&mut self, field: T, seq: Option<i32>) -> &mut Self {
        ...
        if typid == TypeId::of::<Rc<SocketAddr>>() {
            ...
            self.peer_nodeid = Some(rc.deref().clone());
        } else if typid == TypeId::of::<Rc<signature::KeyPair>>() {
            let rc = field.downcast::<Rc<signature::KeyPair>>().unwrap();
    ...
    fn encrypt_with_node(&self, plain: &[u8], cipher: &mut [u8]) -> Result<()> {
        self.node.lock().unwrap().encrypt(
            self.peer_nodeid.as_ref().unwrap(),
            plain,
            cipher
        ).map(|_|())
    }
    ...
    */
    ...
    async fn send_authenticate_request(&mut self, input: &[u8]) -> Result<()> {
        ...
        let kp = signature::KeyPair::random();  // TODO:
        plain.extend_from_slice(kp.public_key().as_bytes());
