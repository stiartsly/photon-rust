...
impl ProxyWorker {
    ...
    pub(crate) fn set_field<T: 'static>(&mut self, field: T, seq: Option<i32>) -> &mut Self {
        ...
        if typid == TypeId::of::<SocketAddr>() {
            ...
            self.peer_nodeid = Some(Rc::new(unwrap!(self.remote_peer).id().clone()));
    ...
    pub(crate) fn encrypt(&self, plain: &[u8], cipher: &mut [u8], nonce: &cryptobox::Nonce) -> Result<()> {
        self.box_.as_ref().unwrap().encrypt(plain, cipher, nonce).map(|_|())
    }

    pub(crate) fn decrypt(&self, cipher: &[u8], plain: &mut [u8], nonce: &cryptobox::Nonce) -> Result<()> {
        self.box_.as_ref().unwrap().decrypt(cipher, plain, nonce).map(|_|())
    }
    ...
    fn save_peer(&self) {
        let val = CVal::Map(vec![
            (
                ...
                CVal::Bytes(unwrap!(self.peer_id).as_bytes().into())
            ),
            (
                CVal::Text(String::from("serverHost")),
                CVal::Text(unwrap!(self.upstream_addr).ip().to_string())
            ),
            (
                CVal::Text(String::from("serverPort")),
                CVal::Integer(unwrap!(self.upstream_addr).port().into())
            ),
            (
                CVal::Text(String::from("serverId")),
                CVal::Bytes(unwrap!(self.peer).nodeid().as_bytes().into())
            )
        ]);
    ...
    async fn announce_peer(&self) {
        ...
        if let Some(url) = peer.alternative_url() {
            info!("-**- ProxyWorker: peer server: {}, domain: {} -**-", self.srv_endp(), url);
        } else {
            info!("-**- ProxyWorker: peer server: {} -**-", self.srv_endp());
        }
    ...
    async fn try_connect(&mut self) -> Result<()> {
        ...
        connection.with_on_authorized_cb(Box::new(move |_: &ProxyConnection, server_pk: &cryptobox::PublicKey, port: u16, domain_enabled: bool| {
            ...
            if let Some(url) = peer.alternative_url() {
                info!("-**- ActiveProxy: peer server: {}, domain: {} -**-", borrowed.srv_endp(), url);
            } else {
                info!("-**- ActiveProxy: peer server: {} -**-", borrowed.srv_endp());
            }
